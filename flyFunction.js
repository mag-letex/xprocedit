...
function getId()
{
    ...
    if(idGlobal.indexOf(newId) == -1 && idGlobal.length <=9999)
    {
        ...
        failedId = newId;
        // console.log("Failed-ID: " + failedId);
        getId();
        }
...
// //DblClckFunc
let atomicSteps = document.querySelectorAll('.atomicStep');
console.log(atomicSteps);
atomicSteps.forEach(function(elem) {
    elem.addEventListener("dblclick", function() {
        ...
        console.log(stepPrimary);
        console.log(stepPrimaryPortGroup);
...
let blaFunc = function (cellView, e, x, y) {
    ...
    $('body').on('mouseup.fly', function (e) {
        ...
        if (x > target.left && x < target.left + paperX.$el.width() && y > target.top && y < target.top + paperX.$el.height()) {
            ...
            // console.log(stepX);
            getId();
            // console.log("New Id-Num: " + newId);
            let stepXmodel = stepX.toJSON();
            // console.log(stepXmodel.stepType);
        ...
        metaActive(cellView);
        // console.log("Global Width: " + globalWidth);
